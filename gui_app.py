...
DEFAULT_AUTO_CHECK_INTERVAL_SEC = 60  # 自動監視の既定間隔秒
...
def safe_filename_component(text: str) -> str:  # ファイル名の安全化
    replaced = re.sub(r"[^A-Za-z0-9._-]", "_", text)  # 許可文字以外を置換
    normalized = re.sub(r"_+", "_", replaced)  # 連続アンダースコアを正規化
    trimmed = normalized.strip("_")  # 前後のアンダースコアを削除
    return trimmed if trimmed else "stream"  # 空の場合は既定名
# 区切り
...
# 区切り
def build_auto_filename(url: str) -> str:  # 自動録画用ファイル名を生成
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")  # タイムスタンプ生成
    label = derive_channel_label(url)  # チャンネルラベルを生成
    return f"auto_{label}_{timestamp}.ts"  # 自動録画ファイル名を返却
# 区切り
...
# 区切り
    def _start_auto_recording(self, url: str) -> None:  # 自動録画開始処理
        ...
        auto_filename = build_auto_filename(normalized_url)  # 自動録画用ファイル名生成
        output_path = resolve_output_path(output_dir, auto_filename)  # 出力パス生成
...
# 区切り
    def closeEvent(self, event: QtGui.QCloseEvent) -> None:  # 終了時処理
        if self.preview_player.playbackState() == QtMultimedia.QMediaPlayer.PlaybackState.PlayingState:  # 再生中判定
            self.preview_player.stop()  # プレビュー停止
