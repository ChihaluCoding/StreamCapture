...
$distExe = Join-Path $projectRoot "dist\配信録画くん.exe"  # 既存EXEのパスを組み立て
if (Test-Path $distExe) {  # 既存EXEがある場合
    try {  # 削除の例外処理開始
        Remove-Item -Force $distExe  # 既存EXEを削除
    } catch {  # 削除失敗時の処理
        Write-Error "dist\配信録画くん.exe を削除できませんでした。実行中なら終了してください。"  # エラー通知
        exit 1  # 失敗終了
    }  # 例外処理の終了
}  # 既存EXE削除の終了
...
$appName = "配信録画くん"  # 出力EXEのアプリ名
$entryScript = "gui_app.py"  # エントリーポイントのスクリプト
$iconPath = "icon.ico"  # EXEとアプリのアイコン
$addData = "icon.ico;."  # 実行時に参照するアイコンを同梱
