...
DEFAULT_RECORDING_QUALITY = DEFAULT_QUALITY  # 録画画質の既定値
DEFAULT_RECORDING_MAX_SIZE_MB = 0  # 録画ファイルの最大サイズ(MB)の既定値
DEFAULT_RECORDING_SIZE_MARGIN_MB = 50  # 録画サイズ切替の余裕(MB)
DEFAULT_AUTO_COMPRESS_MAX_HEIGHT = 0  # 自動圧縮の最大解像度(高さ)の既定値
DEFAULT_AUTO_COMPRESS_FPS = 0  # 自動圧縮のFPS既定値
...
DEFAULT_TIMESHIFT_SEGMENT_HOURS = 0  # タイムシフト分割の既定時
DEFAULT_TIMESHIFT_SEGMENT_MINUTES = 1  # タイムシフト分割の既定分
DEFAULT_TIMESHIFT_SEGMENT_SECONDS = 0  # タイムシフト分割の既定秒
DEFAULT_YOUTUBE_RECORDING_BACKEND = "streamlink"  # YouTube録画方式の既定値
SETTINGS_ORG = "PF"  # 設定の組織名
SETTINGS_APP = "はいろく！"  # 設定のアプリ名
